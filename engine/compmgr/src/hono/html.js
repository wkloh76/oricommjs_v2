...
module.exports = async (...args) => {
  ...
  let lib = {};
  try {
    const jpointer = await import(`./html/jspointer.js`);
    let jptr = jpointer.default;
    // let yy = jptr.get({ objevents: "hhaa" }, "/objevents");

    // let aa;

    // if (typeof jptr === "function") {
    //   aa = jptr.toString();
    // } else if (typeof jptr === "object" && jptr !== null) {
    //   aa = await JSON.stringify(jptr.default, null, 2);
    // } else {
    //   aa = JSON.stringify(jptr);
    // }

    // let { default: jptr } = await import(`./html/jspointer.js`)();
    /**
